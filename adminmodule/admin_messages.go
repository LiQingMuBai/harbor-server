...
func (m *AdminUserModule) DelUserNotice(r *gin.Context) {
	id := m.GetInt(r, "id")
	m.SendResponse(r, common.HTTP_CODE_SUCCESS, adminmodel.MODEL_USER.DelUserNotice(id))
}

func (m *AdminUserModule) SendUserNotice(r *gin.Context) {
	rq := new(adminmodel.UserNoticeMsg)
	m.ConvertObject(r, rq)
	m.SendResponse(r, common.HTTP_CODE_SUCCESS, adminmodel.MODEL_USER.SendUserNotice(rq))
}

func (m *AdminUserModule) MsgList(r *gin.Context) {
	rq := make(adminmodel.P, 0)
	m.ConvertObject(r, &rq)
	m.SendResponse(r, common.HTTP_CODE_SUCCESS, adminmodel.MODEL_USER.MsgList(rq))
}

func (m *AdminUserModule) Delmsg(r *gin.Context) {
	id := m.GetValue(r, "sn_id")
	m.SendResponse(r, common.HTTP_CODE_SUCCESS, adminmodel.SYSTEM_MODEL.Delmsg(id))
}

func (m *AdminUserModule) SendMsg(r *gin.Context) {
	rq := new(adminmodel.CustomMsg)
	m.ConvertObject(r, rq)
	m.SendResponse(r, common.HTTP_CODE_SUCCESS, adminmodel.MODEL_USER.SendMsg(rq))
}

func (m *AdminUserModule) UserByMessage(r *gin.Context) {
	m.SendResponse(r, common.HTTP_CODE_SUCCESS, adminmodel.MODEL_USER.UserByMessage(m.GetInt(r, "uid")))
}

func (m *AdminUserModule) Chatlist(r *gin.Context) {
	rq := make(adminmodel.P, 0)
	m.ConvertObject(r, &rq)
	m.SendResponse(r, common.HTTP_CODE_SUCCESS, adminmodel.MODEL_USER.CustomServiceList(rq))
}
