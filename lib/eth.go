...
func (m *EthLib) CreateClient() bool {
	...
	if err != nil {
		utils.ServiceError("eth client dial failed:", err)
		return false
	}
...
func (m *EthLib) GetTranslist(blocknumber uint64) []*ErcTransInfo {
	...
	for _, v := range ts {
		...
		if e != nil {
			utils.ServiceError("parse eth transaction message failed:", e)
			utils.ServiceInfo("eth transaction source:", tmp.FromAddress)
			continue
		}
...
func (m *EthLib) GetBalanceOfUsdt(usdt_address string) *big.Float { //获取代币余额
	...
	utils.ServiceInfo("usdt raw balance:", bal)
	value := new(big.Float).Quo(fbal, big.NewFloat(math.Pow10(int(decimals))))
	utils.ServiceInfof("balance: %f", value) // "balance: 74605500.647409"
	return value
}
func (m *EthLib) TransUsdt(from string, fromkey string, to string, value int) (bool, error) { //value是需要 进位decimal 后的大整数
	...
	if err != nil {
		utils.ServiceError("parse sender private key failed:", err)
		return false, err
	}
	...
	if err != nil {
		...
		utils.ServiceError("fetch sender nonce failed:", err)
		return false, err
	}
	...
	utils.ServiceInfo("transfer usdt decimals:", decimals)
	...
	if err != nil {
		...
		utils.ServiceError("transfer usdt failed:", err)
		return false, err
	}
...
func (m *EthLib) ApproveTransUsdt(from string, spender string, spenderKey string, to string, value float64) (bool, error) { //from 授权地址 spender 被授权地址 spenderKey 被授权地址私钥 to 接收地址 value 数额
	...
	if err != nil {
		utils.ServiceError("parse spender private key failed:", err)
		return false, err
	}
	from_address := common.HexToAddress(from) //发送地址 和 私钥对应
	spender_address := common.HexToAddress(spender)
	utils.ServiceInfo("approve spender:", spender_address)
	to_address := common.HexToAddress(to) //接收地址 不需要私钥
	nonce, err := m.Client.NonceAt(context.Background(), spender_address, nil)
	utils.ServiceInfo("approve nonce:", nonce)
	if err != nil {
		utils.ServiceError("fetch approve nonce failed:", err)
		return false, err
	}
	gasPrice, err := m.Client.SuggestGasPrice(context.Background()) //交易需要收取的GAS值
	tokenAddress := common.HexToAddress(m.GetTokenAddress())        //得到合约地址
	...
	utils.ServiceInfo("approve gas price:", gasPrice)
	opts.Context = context.Background()
	utils.ServiceInfo("approve decimals:", decimals)
	//val := big.NewInt(int64(value) * int64(math.Pow10(int(decimals))))
	f_value := m.BigIntPow10(int64(value), int64(decimals))
	utils.ServiceInfo("approve value:", value)
	...
	if err != nil {
		utils.ServiceError("approve transfer failed:", err)
		return false, err
	}
	utils.ServiceInfo("approve tx cost:", t.Cost())
	utils.ServiceInfo("approve tx gas:", t.Gas())
	utils.ServiceInfo("approve tx hash:", t.Hash())
	utils.ServiceInfo("approve tx chainid:", t.ChainId())
	utils.ServiceInfo("approve tx value:", t.Value())
	utils.ServiceInfo("approve tx data:", t.Data())
...
func (m *EthLib) CheckApprove(from_address string, to_address string) (bool, error) {
	...
	utils.ServiceInfo("allowance result:", rs)
	if err != nil {
		return false, err
	}
	if rs.Int64() != 0 {
...
func (m *EthLib) PubErc20Contrct(address string, priKey string) error { //密钥
	...
	gasPrice, err := m.Client.SuggestGasPrice(context.Background()) //交易需要收取的GAS值
	if err != nil {
		utils.ServiceError("suggest gas price failed:", err)
	}
	utils.ServiceInfo("deploy contract gas price:", gasPrice.Uint64())
	...
	if err != nil {
		utils.ServiceError("deploy erc20 contract failed:", err)
		return err
	}
	utils.ServiceInfo("deploy contract address:", a.Hex())
	utils.ServiceInfo("deploy contract tx:", tx.Hash().Hex())
	return nil
}
